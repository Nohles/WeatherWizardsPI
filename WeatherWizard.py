...
def getLiveSensorData():
    ...
    s.send("1, "+str(temp)+", "+ str(humidity) +", "+ str(ldr.raw_value) +", "+ str(norain.value) +", "+ str(nomoisture.raw_value) +", "+ str(phnum)+", "+ str(runName)+", ,  " + "\n")
...
def AdvRun(userChoice):
    ...
    timeToEnd = duration
    cursor.execute("Insert INTO Runs Values ("+runID+", 'Active')")
    while True:
        ...
        if elapsed_time > duration:
            ...
            if(s.client_connected):
                ...
                print(rows)
                #s.send("2, "+str(rows[0][0])+ ", 70, 20, 300, 600, 7\n") 
        ...
        if(s.client_connected):
            s.send("1, "+str(temp)+", "+ str(humidity) +", "+ str(ldr.raw_value) +", "+ str(norain.value) +", "+ str(nomoisture.raw_value) +", "+ str(phnum)+", "+str(runName) +", "+str(duration)+", "+ str(elapsed_time)+"\n")
        conn.commit()
        sleep(20)
...
def stillRunning(userChoice):
    ...
    if(rows[0][1] == 'Done'):
        print("done")
        s.send("3, "+str(userChoice[1]) +", Done\n")
    else:
        print("active")
        s.send("3, "+ str(usrChoice[1]) +", Active\n")
